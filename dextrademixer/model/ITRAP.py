...
class ITRAP(ApMHCDeconvolution):
    ...
    def _calculate_ideal_umi_thresholds(self, data):
        ...
        # Calculate expected pMHC for each clonotype
        ct_pep = data.groupby(self.ir_clone_key).filter(lambda x: len(x) >= 10)
        if ct_pep.empty:
            raise ValueError("No clonotypes with more than 10 cells found. By default ITRAP requires at least one "
                             "clonotype with 10 or more cells to calculate ideal UMI threshold.")
